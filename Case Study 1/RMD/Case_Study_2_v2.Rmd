...
A review of the resulting dataframes show that there are missing values for some measurements. The models described in the next section require the data to not have missing values. The option chosen here to handle missing values is to impute them. -100 is used as the dB measurement whenever a missing value is encountered because it was sufficiently far enough away from the range of existing values. Dropping records with missing values was considered, but a significant amount of data was dropped with this method.
