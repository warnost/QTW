...
KNN has several drawbacks for predicting real time location. The model is not easily transferable without the training data, it classifies too many categories, its speed decreases with more observations, and it could have difficulty with outliers.  
...
A classification approach uses a seperate class for every x-y pair, and is probably not the best since it doesn't help the model realize 0-1 and 0-2 are close to one another. We think it would be difficult to get results with this approach.
...
We would recommend additional feature creation and testing other algorithms to see if we can better capture the non-linearity of the data and improve our predictions.  
