...
<font size="4">
<center> **Figure 5:  CART Tree with Default Paramter Values** </center>
</font>
...
<font size="4">
<center> **Figure 6:  CART Tree with Tuned Paramter Values** </center>
<font>
...
Printing the output from `Figure 6` is also quite verbose and we have choosen not to print it here. However, the first split on `perCaps` and the subsequent split for each branch are detailed below:
...
In conclusion, tuning the model parameters significantly improved precision, from 86.84% to 96.75%. The values for the tuned parameters could be cause for concern in regards to overfitting. However, 5-fold cross validation was used in the model, which helps prevent against overfitting. The resulting tree is very deep and complex. The vast amount of data used to train the model makes this complexity possible to improve the model's precision.   
