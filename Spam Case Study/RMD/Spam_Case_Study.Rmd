...
The three graphs below show the effects of our parameter tuning. We compare the effects of different parameter values using a 3D mesh. Higher precision is better, so peaks in the 3D surface represent the best values of precision for a given combination of input parameters. We could use a 2D line to represent the best value of a parameter in isolation, but the 3D surface allows us to see the interaction between parameters.  

In `Figure 1`, we compare complexity and minsplit. The peak appears near complexity = 0 and minsplit = 6. For minsplit values less than 25, there is a plateau in precision from cp=0.23 to cp=0.87. Precision rapidly increases from cp=0.1 to cp=0.8, and again from cp=0.2 to cp=0. For any given value of cp, changing minsplit doesn't seem to have much effect.
```{r}
...
fig
```
In `Figure 2`, we compare complexity and maxdepth. The peak appears near complexity = 0 and maxdepth = 29. For minsplit values less than 30, there is a plateau in precision from cp=0.23 to cp=0.87. Precision rapidly increases from cp=0.1 to cp=0.87, and again from cp=0.23 to cp=0. For any given value of cp, changing maxdepth doesn't seem to have much effect.
...
In `Figure 3` we compare different levels of maxdepth and minsplit. There are many peaks across the 3D plane, suggesting that there are several different levels of these parameters that could help maximize precision. Given the results of figure 1 and 2, I think optimization could be focused on complexity and either minsplit or maxdepth. Both may not be required for this data.  
