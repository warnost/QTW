...
results_tuned = results_combined[which(results_combined$prec==max(results_combined$prec)),]
...
fancyRpartPlot(model_rpart_example$finalModel,main="Figure 1: CART Tree with maxdepth=2")
...
336 different combinations of `minsplit` and `maxdepth` were passed to the the `caret::train` function. The `rpart` method was able to tune `cp` independently. 21 different values of `cp` were evaluated for each combination of `minsplit` and `maxdepth.` This totals 7,056 combinations of `minsplit`, `maxdepth` and `cp` that were evaluated. 
...
```{r}
# tree with defaults
fancyRpartPlot(model_rpart_defaults$finalModel, main="Figure 2. CART Tree with Default Paramter Values")
```
...
fancyRpartPlot(model_rpart_tuned$finalModel, main="Figure 3. CART Tree with Tuned Paramter Values")
```

The output below shows the text output for the winning model that corresponds to the tree in `Figure 3`.

```{r}
model_rpart_tuned$finalModel
```
