...
In `Figure 1`, we compare complexity and minsplit. For minsplit values less than 25, there is a plateau in precision from cp=0.23 to cp=0.87. Precision rapidly increases from cp=0.1 to cp=0.8, and again from cp=0.2 to cp=0. For any given value of cp, changing minsplit doesn't seem to have much effect.
```{r}
...
fig
```
In `Figure 2`, we compare complexity and maxdepth. For maxdepth values less than 30, there is a plateau in precision from cp=0.23 to cp=0.87. Precision rapidly increases from cp=0.1 to cp=0.87, and again from cp=0.23 to cp=0. For any given value of cp, changing maxdepth doesn't seem to have much effect.
...
In `Figure 3` we compare different levels of maxdepth and minsplit. Here we can see the highest precision values when minsplit is low and depth is high. When minsplit is in the 15-25 range, we see larger increases in precision when depth increases from 15 to 20. Past depth = 20, the increases to precision are small for minsplit 15-25. For minsplit 5-15, precision increases across the whole range of depths we tested. Decreases in split generally result in higher precision across different depths, although we see 3 peaks/troughs leading up to the maximum precision.  
...
fig3 <- fig3 %>% layout(
  title = "Figure 3: Change in Precision for different values of Minsplit and Maxdepth",
  scene = list(
